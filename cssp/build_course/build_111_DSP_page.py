...
# 輸出檔案路徑
FILENAME = 'assets/html/course/course_111_DSP.html'

# --- 課程資料來源 ---
# 包含課程的所有資訊，結構類似 JSON，方便日後替換成資料庫讀取
COURSE_DATA = {
    "title_zh": "數位訊號處理",
    "title_en": "Digital Signal Processing",
    "semester": "2022 Fall (111學年度上學期)",
    "tag": "電機專",
    "time_loc": "Mabc @ 工程五館 623 室 (EE623)",
    "prerequisites": "訊號與系統、線性代數",
    "textbooks": [
        "Oppenheim and Schafer, <i>Discrete-Time Signal Processing</i>, 3rd edition"
    ],
    "files": [# 供下載的檔案列表
    ],
    "resources": [ # 外部連結列表
    ...
    "grading": [
        {"item": "作業 (Homework)", "percent": 20, "note": "兩次(含)零分，此部分以零分計"},
        {"item": "期中考 (Midterm)", "percent": 40, "note": ""},
        {"item": "期末考 (Final)", "percent": 40, "note": ""}
    ],
    "office_hours": {
        "teacher": "Tue. 18:30 - 20:30",
        "ta": "Tue. 19:00 - 21:00"
    },
    "tas": [ # 助教名單
        {"name": "邱莉雯", "email": "lili0519@cssp.cn.nctu.edu.tw"}
    ],
    "ta_office": "工程五館 618 室 (分機 54421)",
    "announcements": [ # 公告內容
        {"date": "2022/11/07", "title": "期中考 (Midterm Exam)"},
        {"date": "2023/01/09", "title": "期末考 (Final Exam)"},
        {"date": "2023/01/16", "title": "原始成績公佈 (Raw Grade Post)"},
        {"date": "2023/01/17", "title": "學期成績公佈 (Final Grade Post)"},
        {"date": "置頂", "title": "簽到注意事項", "note": "由於校方作業需要，請學分班的同學每堂課都要簽到。"},
    ],
    "homeworks": [ # 作業列表
        {"title": "HW 1", "due": "2022/10/10", "note": "P2.30, P2.37(a)(b), <del>P3.6</del> (P3.6為第三章，先不用寫)。"},
        {"title": "HW 2", "due": "2022/10/17", "note": "請登入學校 e3 收信"},
        {"title": "HW 3", "due": "2022/10/24", "note": "請登入學校 e3 收信"},
        {"title": "HW 4", "due": "2022/11/07", "note": "請登入學校 e3 收信"},
        {"title": "HW 5", "due": "2022/11/21", "note": "請登入學校 e3 收信"},
        {"title": "HW 6", "due": "2022/11/28", "note": "請登入學校 e3 收信"},
        {"title": "HW 7", "due": "2022/12/12", "note": "請登入學校 e3 收信"},
        {"title": "HW 8", "due": "2022/12/19", "note": "請登入學校 e3 收信"},
    ],
}
...
def _render_grid_items(items, icon_name, color_class="text-primary"):
    ...
    """
    if not items:
        return '<p class="text-muted"><i class="fas fa-info-circle mr-2"></i>本學年暫無詳細列表。請參考 <a href="https://e3.nycu.edu.tw" target="_blank">E3 平台</a>取得最新的作業資訊。</p>'
